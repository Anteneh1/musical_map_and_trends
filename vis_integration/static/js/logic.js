...
// Constants for layer bubble colors
var hex_color_race = "#43e16d";
var hex_color_gender = "#ffa9a9";
var hex_color_genre = "#b2ebff";

/**
 * Perform GET request against endpoint using d3.json
...
function createMaps(features) {
    ...
    var streetmap = L.tileLayer("https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}", {
      attribution: "Music Trends | Map data &copy; <a href=\"https://www.openstreetmap.org/\">OpenStreetMap</a> contributors, <a href=\"https://creativecommons.org/licenses/by-sa/2.0/\">CC-BY-SA</a>, Imagery © <a href=\"https://www.mapbox.com/\">Mapbox</a>",
      maxZoom: 24,
      id: "mapbox.streets",
      accessToken: API_KEY
    });
    ...
    var idx = 0;
    d3.selectAll(".leaflet-control-layers-group-name").each(function(d) {
        // Insert the wrapper container
        var $container = document.createElement("span");
        this.parentNode.insertBefore($container, this);
        $container.setAttribute("class", "key_container");

        //var $groupContainer = d3.select(this).insert("span", ".leaflet-control-layers-group-name").classed("key_container", true);
        var $groupContainer = d3.select("#leaflet-control-layers-group-" + idx + " .leaflet-control-layers-group-label .key_container");
        // Insert the label
        $groupContainer.insert("span").classed("label", true).html("Key:");

        // Insert the color swatch container
        var text = this.innerHTML.split("<")[0];
        hex = text == "Races" ? hex_color_race :
                        text == "Genders" ? hex_color_gender :
                        text == "Genres" ? hex_color_genre : "black";

        $groupContainer.insert("span").classed("key", true)
